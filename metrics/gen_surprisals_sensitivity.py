...
def heldout_surprisals(lang,alpha):
...
for alpha in [.0001,.001,.01,.1,1]:
    ...
    for lang in langs:
        ...
        print(lang)
        metrics += heldout_surprisals(lang,alpha)
    all_metrics.append(metrics)

all_surprisals = [[float(l[2]) for l in list[1:]] for list in all_metrics]

N = len(all_surprisals[0])

f = open('surprisals_sensitivity_{}.tsv'.format(dataset),'w')

for i in range(N):
    print('\t'.join([str(l[i]) for l in all_surprisals]),file=f)
